...
PSQL="psql -X --username=posygresql --dbname=students --no-align --tuples-only -c"
...
echo "$($PSQL "SELECT first_name, last_name, GPA FROM students WHERE major_id IS NULL AND (first_name ILIKE 'D%' OR GPA > 3.0)")"
